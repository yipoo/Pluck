...
`APP_STORE_CONNECT.md` 里面用了一些键盘符号(`⌃⌥⌘⇧`)、箭头(`→`)、勾选(`✓`)、星号(`⭐`)等 Unicode,App Store Connect 后台对部分 Unicode 范围会校验失败。

本文件是**清洁版** —— 只用 ASCII + 中文 + 标准标点。直接复制粘贴不会被拒。
...
**中文版:**
```
拖动选择,自动识别,直接进剪贴板。所有处理都在你的 Mac 上,默认不联网,不收集任何数据。律师、医生、财务可放心用。
```

**英文版:**
```
Drag to select, instant OCR, straight to clipboard. Everything processed on your Mac, no network, no telemetry. Trusted by lawyers, doctors, and finance pros.
```
...
| `「」` | U+300C/D | 中文方括号 | 删除 |
| `『』` | U+300E/F | 中文引号 | 删除 |
| `🚀✨🎉` | 各类 emoji | 表情 | 删除(尤其在 Promotional Text / Keywords) |
...
```bash
# 检查文件里有没有非 BMP 字符或控制字符
python3 -c "
text = open('text.txt').read()
for i, c in enumerate(text):
    n = ord(c)
    if n > 0xFFFF or (n < 0x20 and c not in '\n\t'):
        print(f'位置 {i}: U+{n:04X} 字符 [{c}]')"
```
