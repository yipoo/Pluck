...
```bash
cd /Users/dinglei/MyClaude/snap
swift build         # 构建(产物在 .build/)
swift test          # 跑测试
swift run Snap      # 运行可执行(开发期会显示 Dock 图标,正常)
```
...
5. Bundle Identifier 自动:`com.dinglei.snap`
6. Interface:**SwiftUI**
7. Language:**Swift**
8. 取消勾选 "Use Core Data"、"Include Tests"(我们已有 Tests/)
9. 保存到 `/Users/dinglei/MyClaude/snap/App/`
...
3. 选择 `/Users/dinglei/MyClaude/snap/`(本仓库根)
4. 勾选 `Snap` library product

#### 2.3 替换 App Target 的入口文件

把 Xcode 自动生成的 `SnapApp.swift` 内容替换为引用 Package 的 App:

```swift
import SwiftUI
import Snap   // 我们 Package 里的库

@main
struct SnapAppMain: App {
    var body: some Scene {
        SnapAppScene()       // Package 暴露的 Scene
    }
}
```

(对应需要在 Package 里把 `App.swift` 的 Scene 抽成可复用的 `SnapAppScene`,见代码注释)
...
| `NSScreenCaptureUsageDescription` | String | "Snap 需要屏幕录制权限以截图" | 屏幕录制权限弹窗文案 |
| `NSAppleEventsUsageDescription` | String | "用于剪贴板检测" | (如需)|

#### 2.5 entitlements

新建 `Snap.entitlements`:
...
A:macOS 14+ 需要在 系统设置 → 隐私与安全性 → 屏幕录制 中授予 Snap 权限,再退出 App 重启。
